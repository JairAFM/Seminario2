...
<script>
    ...
    try {
        ...
        // Mostrar en consola lo que devuelve el login
        console.log('Login response:', data);

        // Mostrar una alerta con la respuesta del login
        if (response.ok && data.token) {
            localStorage.setItem('tipoUser', data.data.Tipo_User);
            localStorage.setItem('token', data.token);
            localStorage.setItem('userId', data.data.Id_User);
            alert('Inicio de sesión exitoso. Bienvenido, ' + data.data.Nombres + '!' + data.data.id_user + " tipo " + data.data.Tipo_User);
            location.reload();
        } else {
